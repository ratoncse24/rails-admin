...
class BannersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create banner" do
    assert_difference('Banner.count') do
      post banners_url, params: { banner: { link: @banner.link, status: @banner.status, title: @banner.title } }
    end
  ...
  test "should update banner" do
    patch banner_url(@banner), params: { banner: { link: @banner.link, status: @banner.status, title: @banner.title } }
